...
Cas de base: pour p = 2, j - i = 2\\
	...
	\lstinline{range(i+1, j+1)} équivaut à $\llbracket j-1, j \rrbracket$\\
		...
		\item 
		Lorsque k = j-1,\\
		on appelle \lstinline{tailleMaxRec(a, i, j-2)}, on a $u_{0}$ et \\
		on appelle \lstinline{tailleMaxRec(a, j-1, j)}, on a $u_{1}$
		\item
		Lorsque k = j,\\
		on appelle \lstinline{tailleMaxRec(a, i, j-1)}, on a $u_{1}$ et \\
		on appelle \lstinline{tailleMaxRec(a, j, j)}, on a $u_{0}$
		\end{description}
